...
def get_hnet_model(config, mnet, logger, device):
    ...
    print('called def get_hnet_model() in train_utils')
    #print('input to hyperconfigsutils.get_hnet_model  -.num_tasks',config.num_tasks)
    hnet = sutils.get_hnet_model(config, config.num_tasks, device,mnet.param_shapes)
    ...
    #for i in range(0, 6):
    #   print('hnet.get_task_emb(i) before init', hnet.get_task_emb(i))
    #print('hnet.theta',hnet.theta)
    ...
    print('end of def get_hnet_model() in train_utils')
...
def init_network_weights(all_params, config, logger, chunk_embs=None,
    ...
    Args:
        ...
        net (optional): The network from which the parameters stem come from.
            ...
            batch-norm weights).
    """
    print('called def init_network_weights() initialiing network weights in train_utils.py')
