    ...
    with torch.no_grad():
        ...
        for t in task_range:
            ...
            while N_processed < test_size:
                ...
                if class_nets is not None:
                    theta_ind = theta_saved[t]
                    if config.training_with_hnet:
                        weights_dummy = c_net_hnet.forward(task_id=0, theta=theta_ind, test=True)
                ...
                if config.cl_scenario != 1:
                    if infer_net is not None:
                        ...
                        for e in entrop_to_test:
                            theta_ind = theta_saved[t]
                            weights_c = c_net_hnet.forward(task_id=e, theta=theta_ind, test=True)
                ...
                if class_nets is not None:
                    ...
                    theta_ind = theta_saved[t]
                    # print('theta_saved[0] final test', theta_saved[0])
                    # print('this is here')
                    if config.training_with_hnet:
                        weights_c = c_net_hnet.forward(task_id=inf_task_id, theta=theta_ind, test=True)
...
def train_class_one_t(dhandler_class, dhandlers_rp, dec, d_hnet, net,
    ...
    print('beta for ent', config.class_beta)
    print('cl_scenario', config.cl_scenario)
    print('calc_reg', calc_reg)
    for i in range(training_iterations):
        ...
        if calc_reg:
            ...
            loss_reg = ewc.ewc_regularizer(t, net_hnet.theta, None, hnet=net_hnet, online=True, gamma=1)
            '''       
            loss_reg = config.class_beta * hreg.calc_fix_target_reg(net_hnet, t,
                                                                    ...
                                                                    fisher_estimates=fisher)
            '''
            # print('loss task,loss_reg',loss_task,loss_reg)
    ...
    fish = []
    '''
    if t != 9 and config.class_beta != 0:
        fish = ewc.fisher_mat(task_id=t, data=dhandler_class, params=weights_c, device=device, mnet=net, hnet=net_hnet,
                              empirical_fisher=True, online=True, gamma=1,
                              n_max=-1, regression=False,
                              allowed_outputs=None, logger=None)
    '''
    if t != 9 and config.class_beta != 0:
        fish = ewc.fisher_mat_ewc(task_id=t, data=dhandler_class, params=params_to_regularize, device=device, mnet=net,
                                  hnet=net_hnet, empirical_fisher=True, online=True, gamma=1,
                                  n_max=-1, regression=False,
                                  allowed_outputs=None, logger=None)
    return fish, weights_c
...
def train_tasks(dhandlers_class, dhandlers_rp, enc, dec, d_hnet, class_net,
    ...
    for t in range(0, config.num_tasks):
        ...
        # print('class_net', class_net)
        if class_net is not None:
            if not (config.class_incremental and t == 0):
                print("Training classifier on data handler: ", t)
                print("classsifier")
                fish, weights11 = train_class_one_t(dhandler, dhandlers_rp, dec,
                                                    d_hnet, class_net, device, config, writer, t, fisher=fisher)
        else:
            if t > 0:
                print("Training task inference system on data handler: ", t)
                fish, weights11 = train_class_one_t(dhandler, dhandlers_rp, dec,
                                                    d_hnet, infer_net, device, config, writer, t, fisher=fisher)

        fisher.append(fish)
        net_hnet1 = class_net[1]

        # for memory

        '''
        x11 = []
        params_to_regularize = list(net_hnet1.theta)
        x11.append([d.detach().clone() for d in params_to_regularize])
        z = x11[0][0:17]
        z1 = x11[0][17:]
        theta_saving.append(z)
        if t == config.num_tasks - 1:
            save_linear.append(z1)
            for k1 in range(0, config.num_tasks):
                theta_saving[k1] = theta_saving[k1] + save_linear[0]
        
        
        '''
        # In LSTM_NET_GROW we freeze hidden weights of LSTM and input weights are learnt for each task
        # So, even we store entire set of parameters still its same because hidden state already freezed
        # you can see freezing code in forward method of LSTMs.LSTM_cell
        # To make more memory effcient we can only store input weights because hidden weights are same across the tasks.but both works same
        params_to_regularize = list(net_hnet1.theta)
        ...
        tracemalloc.stop()
        nvmlInit()
        h = nvmlDeviceGetHandleByIndex(4)
        ...
        if not (t == 0 and class_net is None):
            durring_cc = 0
            # test([dhandler], class_net, infer_net, device, config, writer, theta_saving, task_id=t)
    ...
    return during_accs, theta_saving
...
def run(mode='split'):
    ...
    if config.infer_task_id and not config.cl_scenario == 1 and \
        ...
        config.during_accs_inference, ffff = train_tasks(dhandlers_class,
    ...
    class_nets = train_utils.generate_classifier(config,
                                                 dhandlers_class, device)
    ### Train the network.
    config.during_accs_final, theta_saved = train_tasks(dhandlers_class, dhandlers_rp, enc,
                                                        dec, d_hnet, class_nets, device, config, writer, infer_net)

    print("Testing final classifier ...")
    ### Test network.
    test(dhandlers_class, class_nets, infer_net, device, config, writer, theta_saved)
    print(('during', config.during_accs_final))
