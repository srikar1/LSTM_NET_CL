    ...
    for i in ids_to_reg:
        ...
        if inds_of_out_heads is not None:
            ...
            W_predicted = flatten_and_remove_out_heads(mnet, weights_predicted,
                                                       inds_of_out_heads[i])
            # fisher_estimates = flatten_and_remove_out_heads(mnet, fisher_estimates,
            #                                          inds_of_out_heads[i])
        else:
        ...
        # we have W_target and W_predicted of length 4,65,290
        '''
        length = len(W_target)
        chunk_size = 9000
        reg_i = 0
        for z in range(0, length, chunk_size):
            if length - z < chunk_size:
                sub3 = W_target[z:length]
                sub4 = W_predicted[z:length]
                reg_i += (sub3 - sub4).pow(2).sum()
            else:
                add1 = z + chunk_size
                sub1 = W_target[z: add1]
                sub2 = W_predicted[z: add1]
                reg_i += (sub1 - sub2).pow(2).sum()
        # print('reg_i', reg_i)
        '''
        if fisher_estimates is not None:
            _assert_shape_equality(weights_predicted, fisher_estimates[i])
            # print('i am new fisher')
...
def calc_fix_target_reg_online(hnet, task_id, targets=None, dTheta=None, dTembs=None,
                               mnet=None, inds_of_out_heads=None,
                               fisher_estimates=None, prev_theta=None,
                               prev_task_embs=None, batch_size=None, reg_scaling=None):
    r"""This regularizer simply restricts the output-mapping for previous
    ...
    if i == (task_id - 1):
        ...
        if fisher_estimates is not None:
            _assert_shape_equality(weights_predicted, fisher_estimates[i])
            # print('i am new fisher, taskid',task_id)
            FI = torch.cat([w.view(-1) for w in fisher_estimates[i]])
            # len1=len(W_target)
            # FI=FI[:len1]
            reg_i = (FI * (W_target - W_predicted).pow(2)).sum()
...
def calc_fix_target_reg_online_ewc(hnet, task_id, targets=None, dTheta=None, dTembs=None,
                                   mnet=None, inds_of_out_heads=None,
                                   fisher_estimates=None, prev_theta=None,
                                   prev_task_embs=None, batch_size=None, reg_scaling=None):
    assert (isinstance(hnet, CLHyperNetInterface))
    ...
    # print('dTembs is none or not', dTembs)
    ids_to_reg = list(range(1))
    # task_id = task_id-1
    for i in ids_to_reg:
        if dTembs is None:
            weights_predicted = hnet.forward(task_id=task_id)

        if targets is not None:
            target = targets[task_id - 1]
        for k, p in enumerate(weights_predicted):
            reg_i += (fisher_estimates[task_id - 1][k] * (target[k] - weights_predicted[k]).pow(2)).sum()
        '''      
        W_target = torch.cat([w.view(-1) for w in target])
        W_predicted = torch.cat([w.view(-1) for w in weights_predicted])

        if fisher_estimates is not None:
            _assert_shape_equality(weights_predicted, fisher_estimates[task_id - 1])
            FI = torch.cat([w.view(-1) for w in fisher_estimates[task_id - 1]])
            reg_i = (FI * (W_target - W_predicted).pow(2)).sum()
        '''
    return reg_i
