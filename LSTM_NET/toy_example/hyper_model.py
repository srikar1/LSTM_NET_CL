...
class HyperNetwork(nn.Module, CLHyperNetInterface):
    def __init__(self, target_shapes, num_tasks, layers=[50, 100], verbose=True,
        ...
        print('target_shapes',target_shapes)
        ...
        print('self._num_outputs', self._num_outputs)
    ...
    def _gen_layers(self, layers, te_dim, use_bias, no_weights, init_weights,
        ...
        # Hidden layers.
        print('4-------def _gen_layers() in hyper_model--------------')
        self._hidden_dims = []
        prev_dim = te_dim
        ...
        for i, size in enumerate(layers):
            ...
            prev_dim = size
            print('self._hidden_dims',self._hidden_dims)
        self._last_hidden_size = prev_dim

        # Output layers.
        self._out_dims = []
        for i, dims in enumerate(self.target_shapes):
            #print('layers in Output layers.,self._last_hidden_size',layers,self._last_hidden_size)
            nouts = np.prod(dims)
            self._out_dims.append([nouts, self._last_hidden_size])
            if use_bias:
                self._out_dims.append([nouts])
               # print('self._out_dims using bias',self._out_dims)
        if no_weights:
            self._theta = None
            return
        ...
        if init_weights is not None:
            ...
            print('init_weights is None,len(self._theta)',len(self._theta))
    ...
    def forward(self, task_id=None, theta=None, dTheta=None, task_emb=None,
        ...
        print('6------def forward() in hypermodel-----------')
        ...
        if dTheta is not None:
            ...
            weights = []
            print('dtheta is requireddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd')
        ...
        if ext_inputs is not None:
            ...
            #print('ext_inputs.size()',ext_inputs.size())
            batch_size = ext_inputs.shape[0]
            #print('batchsize,self._te_dim',batch_size,self._te_dim)
            task_emb = task_emb.expand(batch_size, self._te_dim)
            #print('task_emb.size()',task_emb.size())
        ...
        for i in range(len(self._hidden_dims), len(self._theta_shapes),
            ...
            #print('W.size(),weights[i]',W.size(),weights[i].size())
    ...
    def apply_hyperfan_init(self, method='in', use_xavier=False,
        ...
        **Variance of the hypernet input**
        ...
        print('hyperfan init in hyper model')
